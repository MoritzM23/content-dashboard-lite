...
export type CoachScene = 'planung' | 'analytics';

interface CoachChatProps {
  scene: CoachScene;
  title: string;
  subtitle: string;
  /** Beispiel-Prompts oben bei leerem Chat. Erster Klick startet die Konversation. */
  quickPrompts: string[];
  /** Tag-Liste fuer die Capability-Pills. */
  capabilities: string[];
}
  ...
  async function send(text: string) {
    ...
    try {
      const res = await fetch(`${API_BASE}/coach/chat`, {
        ...
        body: JSON.stringify({ messages: next, scene }),
      });
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="label-mono text-primary">{title}</span>
      ...
      {messages.length === 0 && (
        <div className="px-5 py-4 space-y-3">
          <p className="text-sm text-foreground/85 leading-relaxed">
            Liest beim ersten Send automatisch deine letzte AI-Analyse, Themen-Cluster,
            KPIs und Wettbewerbs-Top-Overview. Antwortet in Markdown.
          </p>
